...
// --- Static wiki content ---
// Article bodies stay in English for now — they are educational long-form
// content that community contributors can translate per locale. The category
// labels and page chrome ARE translated via the i18n layer.
$wikiCategories = [
    'basics' => ['label' => t_raw('wiki.cat.basics'),  'icon' => 'fa-book-open'],
    'macros' => ['label' => t_raw('wiki.cat.macros'),  'icon' => 'fa-drumstick-bite'],
    'micros' => ['label' => t_raw('wiki.cat.micros'),  'icon' => 'fa-pills'],
    'habits' => ['label' => t_raw('wiki.cat.habits'),  'icon' => 'fa-heart'],
];
        ...
        <div class="wiki-container">
            ...
            <div class="wiki-toolbar">
                <div class="wiki-search">
                    ...
                    <input type="text" id="wikiSearch" placeholder="<?= t('wiki.search.placeholder') ?>"
                        aria-label="<?= t('wiki.search.aria') ?>">
                </div>
                <div class="wiki-filters" id="wikiFilters">
                    <button type="button" class="wiki-pill active" data-cat="all"><?= t('wiki.filter.all') ?></button>
            ...
            <div class="wiki-grid" id="wikiGrid">
                <?php foreach ($wikiArticles as $a):
                    // Translate title/summary/body by the article key. The English
                    // text in $wikiArticles is the canonical fallback: if a locale
                    // file lacks the key, t_raw() returns the key literal and we
                    // fall back to the in-array English so nothing renders blank.
                    $aKey = $a['key'] ?? '';
                    $titleKey   = 'wiki.art.' . $aKey . '.title';
                    $summaryKey = 'wiki.art.' . $aKey . '.summary';
                    $bodyKey    = 'wiki.art.' . $aKey . '.body';
                    $aTitle   = $aKey ? t_raw($titleKey)   : $a['title'];
                    if ($aTitle === $titleKey)     $aTitle   = $a['title'];
                    $aSummary = $aKey ? t_raw($summaryKey) : $a['summary'];
                    if ($aSummary === $summaryKey)  $aSummary = $a['summary'];
                    $aBody    = $aKey ? t_raw($bodyKey)    : $a['body'];
                    if ($aBody === $bodyKey)        $aBody    = $a['body'];
                    $aMins    = t_raw('wiki.read_time', ['n' => (int) $a['mins']]);

                    $search = strtolower($aTitle . ' ' . $aSummary . ' ' . ($wikiCategories[$a['cat']]['label'] ?? ''));
                    ?>
                    <article class="wiki-article" data-cat="<?= htmlspecialchars($a['cat']) ?>"
                        ...
                        <button type="button" class="wiki-article-head" aria-expanded="false">
                            ...
                            <span class="wiki-article-titles">
                                <span class="wiki-article-title"><?= htmlspecialchars($aTitle) ?></span>
                                <span class="wiki-article-summary"><?= htmlspecialchars($aSummary) ?></span>
                            </span>
                            <span class="wiki-article-meta"><i class="far fa-clock"></i> <?= htmlspecialchars($aMins) ?></span>
                            <i class="fas fa-chevron-down wiki-chevron"></i>
                        </button>
                        <div class="wiki-article-body">
                            <div class="wiki-article-body-inner">
                                <div class="wiki-article-content"><?= $aBody ?></div>
                            </div>
                        </div>
                    </article>
                <?php endforeach; ?>
            ...
            <p class="wiki-no-results" id="wikiNoResults" hidden><?= t('wiki.no_results') ?></p>
