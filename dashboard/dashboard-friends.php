...
if (!$isLoggedIn) {
    ...
    <head>
        ...
        <title>Friends | BitBalance</title>
        <?php
    ...
    <body class="<?= htmlspecialchars($bodyClass, ENT_QUOTES) ?>">
        ...
        <main class="dashboard-content">
            <section class="friends-locked">
                ...
                <span class="friends-kicker"><i class="fas fa-user-friends"></i> Account feature</span>
                <h1>Sign in to use Friends</h1>
                <p>Friends needs your BitBalance identity so requests, privacy, and shared progress stay tied to real accounts.</p>
                <div class="friends-locked__actions">
                    <a href="<?= BASE_URL ?>login.php" class="btn-tactile btn-tactile--primary">Sign in</a>
                    <a href="<?= BASE_URL ?>signup.php" class="btn-tactile btn-tactile--ghost">Create account</a>
                </div>
            </section>
        </main>
    ...
    <article class="friend-card" data-user-id="<?= $uid ?>" data-request-id="<?= $reqId ?>">
        ...
        <div class="friend-card__body">
            ...
            <div class="friend-card__stats">
                <?php if ($streak > 0): ?>
                    <span class="friend-card__stat"><i class="fas fa-fire"></i> <?= $streak ?>d</span>
                <?php endif; ?>
                <?php if ($weekly !== null): ?>
                    <span class="friend-card__stat"><i class="fas fa-bolt"></i> <?= number_format($weekly) ?> XP / 7d</span>
                <?php endif; ?>
            </div>
        </div>
        <div class="friend-card__actions">
            ...
            <?php elseif ($context === 'pending_out'): ?>
                <span class="friend-card__hint">Waiting…</span>
                <button class="btn-tactile btn-tactile--ghost js-cancel" data-csrf="<?= htmlspecialchars($csrf, ENT_QUOTES) ?>">
                    Cancel
                </button>
            <?php elseif ($context === 'find'): ?>
                <?php if ($rel === 'friends'): ?>
                    <span class="friend-card__hint friend-card__hint--ok"><i class="fas fa-check"></i> Friends</span>
                <?php elseif ($rel === 'pending_out'): ?>
                    <span class="friend-card__hint">Pending</span>
...
function fr_render_leaderboard_row(array $row, string $period): void
{
    ...
    $scoreLabel = $period === 'all_time' ? 'Total XP' : 'XP / 7d';
    $secondaryScore = $period === 'all_time'
        ? number_format($weekly) . ' XP / 7d'
        : number_format($total) . ' total XP';
    ?>
    <article class="leaderboard-row<?= $isYou ? ' leaderboard-row--you' : '' ?>" data-user-id="<?= (int) ($row['user_id'] ?? 0) ?>">
        ...
        <div class="leaderboard-main">
            <div class="leaderboard-name-line">
                <h3><?= $name ?></h3>
                <?php if ($isYou): ?><span class="leaderboard-you-chip">You</span><?php endif; ?>
            </div>
            <div class="leaderboard-meta">
                <span><i class="fas fa-shield-alt"></i> Lv <?= $level ?></span>
                <span><i class="fas fa-fire"></i> <?= $streak ?>d streak</span>
        ...
        <div class="friends-container" data-active-tab="<?= htmlspecialchars($activeTabFromUrl, ENT_QUOTES) ?>">
            <section class="friends-hero">
                <div class="friends-hero__copy">
                    <span class="friends-kicker"><i class="fas fa-user-friends"></i> Your circle</span>
                    <h1>Friends</h1>
                    <p>Build a small accountability crew. Add people you trust to push each other through streaks.</p>
                </div>
                <div class="friends-hero__metrics">
                    <div class="friends-metric">
                        <span class="friends-metric__label">Friends</span>
                        <strong><?= count($friends) ?></strong>
                    </div>
                    <div class="friends-metric">
                        <span class="friends-metric__label">Pending</span>
            ...
            <section class="friends-panel" data-panel="pending" <?= $activeTabFromUrl === 'pending' ? '' : 'hidden' ?>>
                <div class="friends-subhead">Requests for you</div>
                ...
                <div class="friends-subhead friends-subhead--spaced">Requests you sent</div>
            ...
            <section class="friends-panel" data-panel="leaderboard" <?= $activeTabFromUrl === 'leaderboard' ? '' : 'hidden' ?>>
                <div class="leaderboard-panel-head">
                    <div>
                        <div class="friends-subhead">Friend rankings</div>
                        <p class="leaderboard-copy">Compete with your accepted friends. Your own row is included.</p>
                    </div>
                    <div class="leaderboard-period-toggle" role="group" aria-label="Leaderboard period">
                        <button
                            type="button"
                ...
                <div class="friends-empty friends-empty--inline leaderboard-solo-state" <?= count($leaderboardRows) <= 1 ? '' : 'hidden' ?>>
                    ...
                    <p>Your leaderboard starts with you. Find people you trust and compare weekly progress.</p>
        ...
        (function () {
            ...
            function renderLeaderboardRow(u, period) {
                ...
                const rankContent = rank <= 3
                    ? '<i class="fas ' + (rank === 1 ? 'fa-trophy' : 'fa-medal') + '"></i>'
                    : String(rank);
                const scoreLabel = period === 'all_time' ? 'Total XP' : 'XP / 7d';
                const secondaryScore = period === 'all_time'
                    ? formatNumber(u.weekly_xp) + ' XP / 7d'
                    : formatNumber(u.total_xp) + ' total XP';
                    ...
                    <article class="leaderboard-row${isYou ? ' leaderboard-row--you' : ''}" data-user-id="${toNumber(u.user_id)}">
                        ...
                        <div class="leaderboard-main">
                            <div class="leaderboard-name-line">
                                ...
                                ${isYou ? '<span class="leaderboard-you-chip">You</span>' : ''}
                            </div>
                            <div class="leaderboard-meta">
                                <span><i class="fas fa-shield-alt"></i> Lv ${formatNumber(u.current_level)}</span>
                                <span><i class="fas fa-fire"></i> ${formatNumber(u.logging_streak)}d streak</span>
                                <span><i class="fas fa-bolt"></i> ${secondaryScore}</span>
                            </div>
                        </div>
                        <div class="leaderboard-score">
            ...
            async function loadLeaderboard(period) {
                ...
                leaderboardList.innerHTML = '<div class="friends-empty friends-empty--inline"><p>Loading leaderboard...</p></div>';
                try {
                    ...
                    leaderboardList.innerHTML = '<div class="friends-empty friends-empty--inline"><p>Leaderboard failed: ' + escapeHtml(err.message) + '</p></div>';
                } finally {
            ...
            container.addEventListener('click', async e => {
                ...
                try {
                    if (e.target.closest('.js-accept'))         { await postAction('accept', { request_id: reqId }); card.remove(); }
                        ...
                        btn.outerHTML = '<span class="friend-card__hint">Pending</span>';
                    }
                        ...
                        btn.outerHTML = '<span class="friend-card__hint friend-card__hint--ok"><i class="fas fa-check"></i> Friends</span>';
                    }
                } catch (err) {
            ...
            async function runSearch(q) {
                if (!q || q.trim().length < 2) {
                    resultsGrid.innerHTML = '<div class="friends-empty friends-empty--inline"><p>Start typing to find someone.</p></div>';
                    return;
                }
                resultsGrid.innerHTML = '<div class="friends-empty friends-empty--inline"><p>Searching…</p></div>';
                try {
