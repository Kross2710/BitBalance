...
$activePage = 'forums'; // Set the active page for the sidebar

$forumPosts = getAllPosts();
$forumComments = getAllComments();
$forumLikes = getAllLikes();

// Success message handling for post updates
if (isset($_GET['success'])) {
    $success_message = htmlspecialchars($_GET['success'] ?? '');
} else {
    $success_message = '';
}
// Error message handling for post updates
if (isset($_GET['error'])) {
...
// Success message handling for comment updates
if (isset($_GET['csuccess'])) {
    $csuccess_message = htmlspecialchars($_GET['csuccess'] ?? '');
} else {
    $csuccess_message = '';
}
// Error message handling for post updates
if (isset($_GET['cerror'])) {
    $cerror_message = htmlspecialchars($_GET['cerror'] ?? '');
} else {
    $cerror_message = '';
}
?>
...
<body>
    ...
    <main>
                    ...
                    <tbody>
                        <?php foreach ($forumPosts as $post): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars(date('Y-m-d H:i:s', strtotime($post['date_posted']))); ?>
                                </td>
                                <td>
                                    <button type="submit"
                                        style="background:#e55039;color:#fff;border:none;border-radius:4px;padding:4px 10px;cursor:pointer;">
                                        <a href="edit-post.php?post_id=<?php echo htmlspecialchars($post['post_id']); ?>"
        ...
        <div class="main-content">
            ...
            <div class="user-list">
                <div class="search-filter-bar">
                    <input id="searchInput1" type="text" placeholder="Search comments...">
                </div>
                ...
                <?php endif; ?>
                <table id="user-table1" class="user-table">
                    <thead>
                        <tr>
                            <th>CID</th>
                            <th>PID</th>
                    ...
                    <tbody>
                        <?php foreach ($forumComments as $comment): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars(ucfirst($comment['status'])); ?></td>
                                <td><?php echo htmlspecialchars(date('Y-m-d H:i:s', strtotime($comment['date_posted']))); ?>
                                </td>
                                <td>
                                    <button type="submit"
                                        style="background:#e55039;color:#fff;border:none;border-radius:4px;padding:4px 10px;cursor:pointer;">
                                        <a href="edit-comment.php?comment_id=<?php echo htmlspecialchars($comment['comment_id']); ?>"
                                            style="color: white; text-decoration: none;">Edit</a>
                                    </button>
                                </td>
                            </tr>
                ...
                <script>
                    $(function () {
                        // init DataTable without its own search box
                        const table = $('#user-table1').DataTable({
                            dom: 'tip', // just table, info, pagination
                            pagingType: 'simple_numbers',
                            pageLength: 10,
                            search: {
        ...
        <div class="main-content">
            <h2 style="margin-bottom: 20px;">Likes</h2>
            <div class="user-list">
                <div class="search-filter-bar">
                ...
                <table id="user-table2" class="user-table">
                    ...
                    <tbody>
                        <?php foreach ($forumLikes as $like): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($like['like_id']); ?></td>
                                <td><?php echo htmlspecialchars($like['type']); ?></td>
                                <td><?php echo htmlspecialchars($like['target_id']); ?></td>
                                <td><?php echo htmlspecialchars(date('Y-m-d H:i:s', strtotime($like['date_liked']))); ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                ...
                <script>
                    $(function () {
                        // init DataTable without its own search box
                        const table = $('#user-table2').DataTable({
                            dom: 'tip', // just table, info, pagination
                            pagingType: 'simple_numbers',
                            pageLength: 10,
                            search: {
